<template>
  <div style="min-height:1000px">
    <el-button type="success" @click="run_rand">生成随机数</el-button>
    <div>
        {{random_num}}
    </div>
  </div>
...
export default {
  ...
  methods: {
    run_rand() {
        let user_surname=Array('赵', '钱', '孙', '李', '周', '吴', '郑', '王', '冯', '陈', '褚', '卫', '蒋', '沈', '韩', '杨', '朱', '秦', '尤', '许', '何', '吕', '施', '张', '孔', '曹', '严', '华', '金', '魏', '陶', '姜', '戚', '谢', '邹',
            ...
            '寇', '广', '禄', '阙', '东', '欧', '利', '师', '巩', '聂', '关', '荆', '司马', '上官', '欧阳', '夏侯', '诸葛', '闻人', '东方', '赫连', '皇甫', '尉迟', '公羊', '澹台', '公冶', '宗政', '濮阳', '淳于', '单于', '太叔','吴',
            ...
            let letter=Array('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z')
            let user_data=new Array()
      for (let index = 0; index < this.user_num; index++) {
          user_data[index]={
              user_pet_name:user_surname[parseInt(Math.random() * (user_surname.length), 10)]+name[parseInt(Math.random() * (name.length), 10)]+name[parseInt(Math.random() * (name.length), 10)],
              user_name:letter[parseInt(Math.random() * (letter.length), 10)]+letter[parseInt(Math.random() * (letter.length), 10)]+letter[parseInt(Math.random() * (letter.length), 10)]+letter[parseInt(Math.random() * (letter.length), 10)]+letter[parseInt(Math.random() * (letter.length), 10)],
